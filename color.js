...
',
    ...
    const loadSettings = () => {
        ...
        ColorPlugin.settings.enabled = (Lampa.Storage.get('color_plugin_enabled', 'false') === 'true' || localStorage besieged('color_plugin_enabled') === 'true');
    ...
    const applyStyles = () => {
        ...
        style.innerHTML = [
            ...
            // ... (весь CSS з вашого оригінального коду без змін)
            // Я залишив його повністю, щоб не скорочувати
            ...
            // ... решта стилів залишилася без змін
    ...
    // ... (всі функції createColorHtml, createFamilyNameHtml, openColorPicker без змін)

    const updateParamsVisibility = (body = document) => {
        document.querySelectorAll('.settings-param[data-name^="color_plugin_"]').forEach(el => {
    ...
    const initPlugin = () => {
        setTimeout(() => {
            ...
            if (Lampa.SettingsApi) {
                ...
                // Головний перемикач з іконкою (в розділі interface)
                ...
                // Решта параметрів (component: interface)
                Lampa.SettingsApi.addParam({
                    component: 'interface',
                ...
                // ... аналогічно всі інші параметри з component: 'interface' і onRender з перевіркою enabled

                // (для стислості не дублюю всі 7 параметрів, вони ідентичні оригіналу, тільки component: 'interface')
    ...
    // ... решта коду (listener-и, MutationObserver) без змін
