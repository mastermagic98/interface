...
function snapshotInitialState() {
    if (ColorPluginRuntime.applied) return;

    var root = document.documentElement;

    ColorPluginRuntime.cssVars = {
        main: root.style.getPropertyValue('--main-color'),
        rgb: root.style.getPropertyValue('--main-color-rgb'),
        accent: root.style.getPropertyValue('--accent-color')
    };

    document.querySelectorAll('[style]').forEach(function (el) {
        ColorPluginRuntime.inlineStyles.set(el, el.getAttribute('style'));
    });

    document.querySelectorAll('svg path, svg rect, svg circle').forEach(function (el) {
        ColorPluginRuntime.svgAttrs.set(el, {
    ...
    // 1. Видалення style-tag
    var style = document.getElementById('color-plugin-styles');
    if (style) style.remove();

    // 2. Відновлення CSS variables
    var root = document.documentElement;
    root.style.setProperty('--main-color', ColorPluginRuntime.cssVars.main || '');
    root.style.setProperty('--main-color-rgb', ColorPluginRuntime.cssVars.rgb || '');
    root.style.setProperty('--accent-color', ColorPluginRuntime.cssVars.accent || '');

    // 3. Відновлення inline-style
    ColorPluginRuntime.inlineStyles.forEach(function (value, el) {
        if (value === null) el.removeAttribute('style');
        else el.setAttribute('style', value);
    });
    ColorPluginRuntime.inlineStyles.clear();

    // 4. Відновлення SVG
    ColorPluginRuntime.svgAttrs.forEach(function (attrs, el) {
        if (attrs.fill === null) el.removeAttribute('fill');
        else el.setAttribute('fill', attrs.fill);

        if (attrs.stroke === null) el.removeAttribute('stroke');
        else el.setAttribute('stroke', attrs.stroke);
    });
    ColorPluginRuntime.svgAttrs.clear();

    ColorPluginRuntime.applied = false;
}
...
function saveSettings() {
    ...
    // Функція для оновлення SVG-іконок (зірки та інші)
    ...
    ColorPluginRuntime.applied = true;
