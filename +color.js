...
function resetStyles() {
    // 1. Видаляємо стилі плагіна
    var style = document.getElementById('color-plugin-styles');
    if (style) style.remove();

    // 2. Скидаємо CSS-змінні
    var root = document.documentElement;
    root.style.removeProperty('--main-color');
    root.style.removeProperty('--main-color-rgb');
    root.style.removeProperty('--accent-color');

    // 3. Очищаємо inline-стилі, які змінював плагін
    document
        .querySelectorAll('[style*="var(--main-color"]')
        .forEach(function (el) {
            el.removeAttribute('style');
        });

    // 4. Перемалювати Settings та UI
    if (Lampa.Settings && Lampa.Settings.render) {
        Lampa.Settings.render();
    }
}
    ...
    // Якщо плагін вимкнено — повний reset без перезавантаження
    if (!ColorPlugin.settings.enabled) {
        if (typeof resetStyles === 'function') {
            resetStyles();
        } else {
    ...
    style.innerHTML = [
        ...
        /* Базове фарбування активних елементів */
        '.menu__item.focus, .menu__item.traverse, .menu__item:hover,' +
        '.full-person.focus, .full-start__button.focus,' +
        ...
        '.radio-item.focus, .lang__selector-item.focus,' +
        '.modal__button.focus {' +
