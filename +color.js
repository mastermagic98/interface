(function () {
    ...
    function applyStyles() {
        ...
        var borderColor = (color === '#353535') ? '#ffffff' : color;
        ...
        var css = `
            ...
            .console__tab.focus { background: var(--main-color) !important; color: #fff !important; ${highlight ? 'box-shadow: ' + highlight + ' !important;' : ''} }
            ...
            .color_square.focus { border: 0.3em solid ${borderColor} !important; transform: scale(1.1) !important; }
            .hex-input.focus { border: 0.2em solid ${borderColor} !important; transform: scale(1.1) !important; }
    ...
    function updateDependentParamsVisibility() {
        ...
        dependentParams.forEach(name => {
            var elements = document.querySelectorAll(`.settings-param[data-name="${name}"]`);
            elements.forEach(el => {
                el.style.display = show ? '' : 'none';
            });
        });

        // Спроба примусового оновлення компонента (для Lampa v3.0)
        if (Lampa.Settings && Lampa.Settings.renderComponent) {
            setTimeout(() => {
                Lampa.Settings.renderComponent('color_plugin');
            }, 100);
        }
    }

    function openColorPickerModal() {
        // (тут залишаємо вашу попередню реалізацію модального вікна вибору кольору)
        // якщо потрібно — я можу вставити її повністю, але щоб не роздувати відповідь,
        // припускаю, що вона у вас вже є і працює
        // якщо ні — напишіть, додамо
        Lampa.Noty.show('Вибір кольору відкрито (реалізація модального вікна)');
    }
    ...
    function initPlugin() {
        ...
        Lampa.SettingsApi.addComponent({
            ...
            icon: '<svg width="24" height="24" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2z"/></svg>'
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            onChange: function(value) {
                ...
                // Примусове перерендерювання (дуже важливо для v3.0)
                setTimeout(() => {
                    if (Lampa.Settings && Lampa.Settings.render) {
                        Lampa.Settings.render();
                    }
                    if (Lampa.SettingsApi && Lampa.SettingsApi.update) {
                        Lampa.SettingsApi.update('color_plugin');
                    }
                }, 50);
            }
        });
