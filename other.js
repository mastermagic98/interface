(function() {
    ...
    /* --------------------------------------------------------------
       1. Переклади (лише українська, російська, англійська)
       -------------------------------------------------------------- */
    Lampa.Lang.add({
        maxsm_themes: {
            ru: "Темы",
    ...
    /* --------------------------------------------------------------
       2. fix_lang() – переклади статусів серіалів
       -------------------------------------------------------------- */
    function fix_lang() {
        Lampa.Lang.add({
    ...
    /* --------------------------------------------------------------
       3. forall() – універсальні стилі та шаблони карток
       -------------------------------------------------------------- */
    function forall() {
        /* шаблон звичайної картки (рік над назвою) */
        ...
        /* шаблон картки епізоду (без футера, рік/назва на картці) */
        ...
        var css = "\n<style id=\"maxsm_themes_forall\">\n" +
            ...
            ".full-start__rate{border-radius:.25em;padding:.3em;background:rgba(0,0,0,.3);}\n" +
            ".full-start__pg,.full-start__status{font-size:1em;background:#fff;color:#000;}\n" +
            /* заголовок картки */
            ".card__title{height:3.6em;text-overflow:ellipsis;-o-text-overflow:ellipsis;overflow:hidden;-webkit-line-clamp:3;line-clamp:3;}\n" +
            /* рік */
            ".card__age{position:absolute;right:0;bottom:0;z-index:10;background:rgba(0,0,0,.6);color:#fff;font-weight:700;padding:.4em .6em;border-radius:.48em 0 .48em 0;line-height:1;font-size:1em;}\n" +
            /* рейтинг */
            ".card__vote{position:absolute;right:0;top:0;background:rgba(0,0,0,.6);color:#fff;font-weight:700;border-radius:0 .34em 0 .34em;line-height:1;font-size:1.4em;}\n" +
            /* іконки */
            ".card__icons{position:absolute;top:2em;left:0;display:flex;justify-content:center;background:rgba(0,0,0,.6);color:#fff;border-radius:0 .5em .5em 0;}\n" +
            ".card__icons-inner{background:transparent;}\n" +
            /* маркер закладок */
            ".card__marker{position:absolute;left:0;top:4em;background:rgba(0,0,0,.6);border-radius:0 .5em .5em 0;font-weight:700;font-size:1em;padding:.4em .6em;display:flex;align-items:center;line-height:1.2;max-width:min(12em,95%);box-sizing:border-box;}\n" +
            /* якість */
            ".card__quality{position:absolute;right:0;bottom:2.4em;padding:.4em .6em;color:#fff;font-weight:700;font-size:1em;border-radius:.5em 0 0 .5em;text-transform:uppercase;}\n" +
            ...
            /* постер */
            ".full-start__background.loaded{opacity:.8;}.full-start__background.dim{opacity:.2;}\n" +
            /* скруглення */
            ".explorer__files .torrent-filter .simple-button{font-size:1.2em;border-radius:.5em;}\n" +
            ".full-review-add,.full-review,.extensions__item,.extensions__book-add,.search-source,.bookmarks-folder__layer,.bookmarks-folder__body,.card__img,.card__promo,.full-episode--next .full-episode__img:after,.full-episode__img img,.full-episode__body,.full-person__photo,.card-more__box,.full-start__button,.simple-button,.register{border-radius:.5em;}\n" +
            ".extensions__item.focus::after,.extensions__book-add.focus::after,.full-episode.focus::after,.full-review-add.focus::after,.card-parser.focus::after,.card-episode.focus .full-episode::after,.card-episode.hover .full-episode::after,.card.focus .card__view::after,.card.hover .card__view::after,.card-more.focus .card-more__box::after,.register.focus::after{border-radius:1em;}\n" +
            ".search-source.focus,.simple-button.focus,.menu__item.focus,.menu__item.traverse,.menu__item.hover,.full-start__button.focus,.full-descr__tag.focus,.player-panel .button.focus,.full-person.selector.focus,.tag-count.selector.focus{border-radius:.5em;}\n" +
            ".menu__item.focus{border-radius:0 .5em .5em 0;}\n" +
            ".menu__list{padding-left:0;}\n" +
            ".menu__item.focus .menu__ico{-webkit-filter:invert(1);filter:invert(1);}\n" +
            "</style>\n";

        Lampa.Template.add('forall_style_css', css);
        $('body').append(Lampa.Template.get('forall_style_css', {}, true));
    }

    /* --------------------------------------------------------------
       4. incardtemplate() – альтернативний шаблон детальної сторінки
       -------------------------------------------------------------- */
    function incardtemplate() {
        var enabled = localStorage.getItem('maxsm_themes_incardtemplate') === 'true';
        if (!enabled) return;
    ...
    /* --------------------------------------------------------------
       5. translate_tv() – заміна мітки TV
       -------------------------------------------------------------- */
    function translate_tv() {
        var caption = Lampa.Lang.translate('maxsm_themes_tvcaption');
        var enabled = localStorage.getItem('maxsm_themes_translate_tv') === 'true';
        ...
        var style = enabled ?
            ...
            ".card__type::after{content:'" + caption + "';color:#fff;position:absolute;left:0;top:0;padding:.4em .6em;border-radius:.4em 0 0 .4em;font-weight:700;}" :
            ".card__type{position:absolute;bottom:auto;left:0;right:auto;top:0;background:rgba(0,0,0,.6);color:#fff;font-weight:700;padding:.4em .6em;border-radius:.4em 0 0 .4em;line-height:1;font-size:1em;}" +
            ".card--tv .card__type{color:#fff;}";

        $('body').append('<style id="maxsm_themes_translate_tv">' + style + '</style>');
    }

    /* --------------------------------------------------------------
       6. animations() – плавні анімації фокусу
       -------------------------------------------------------------- */
    function animations() {
        var enabled = localStorage.getItem('maxsm_themes_animations') === 'true';
        ...
        var css = "\n<style id=\"maxsm_themes_animations\">\n" +
            ...
            ".extensions__item,.extensions__book-add,.full-review-add,.full-review,.tag-count,.full-person,.full-episode,.simple-button,.full-start__button,.items-cards .selector,.card-more,.explorer-card__head-img.selector,.card-episode{transform:scale(1);transition:transform .3s ease;}\n" +
            ".extensions__item.focus,.extensions__book-add.focus,.full-review-add.focus,.full-review.focus,.tag-count.focus,.full-person.focus,.full-episode.focus,.simple-button.focus,.full-start__button.focus,.items-cards .selector.focus,.card-more.focus,.explorer-card__head-img.selector.focus,.card-episode.focus{transform:scale(1.03);}\n" +
    ...
    /* --------------------------------------------------------------
       7. bigbuttons() – ширші кнопки на мобілці
       -------------------------------------------------------------- */
    function bigbuttons() {
        var enabled = localStorage.getItem('maxsm_themes_bigbuttons') === 'true';
    ...
    /* --------------------------------------------------------------
       8. Ініціалізація плагіну
       -------------------------------------------------------------- */
    function startPlugin() {
        /* значення за замовчуванням */
        if (!localStorage.getItem('maxsm_themes_animations'))   localStorage.setItem('maxsm_themes_animations',   'true');
        if (!localStorage.getItem('maxsm_themes_translate_tv'))localStorage.setItem('maxsm_themes_translate_tv','true');
        if (!localStorage.getItem('maxsm_themes_incardtemplate'))localStorage.setItem('maxsm_themes_incardtemplate','true');
        if (!localStorage.getItem('maxsm_themes_bigbuttons'))  localStorage.setItem('maxsm_themes_bigbuttons',  'false');
        ...
        Lampa.SettingsApi.addParam({
            component: 'maxsm_themes',
            param: { name: 'maxsm_themes_animations',   type: 'trigger', default: true },
            field: { name: Lampa.Lang.translate('maxsm_themes_animations'), description: '' },
            onChange: function(){ animations(); }
        });
        Lampa.SettingsApi.addParam({
            component: 'maxsm_themes',
            param: { name: 'maxsm_themes_translate_tv', type: 'trigger', default: true },
            field: { name: Lampa.Lang.translate('maxsm_themes_translate_tv'), description: '' },
            onChange: function(){ translate_tv(); }
        });
        Lampa.SettingsApi.addParam({
            component: 'maxsm_themes',
            param: { name: 'maxsm_themes_incardtemplate', type: 'trigger', default: true },
            field: { name: Lampa.Lang.translate('maxsm_themes_incardtemplate'), description: '' },
            onChange: function(){ window.location.reload(); }
        });
        Lampa.SettingsApi.addParam({
            component: 'maxsm_themes',
            param: { name: 'maxsm_themes_bigbuttons',   type: 'trigger', default: false },
            field: { name: Lampa.Lang.translate('maxsm_themes_bigbuttons'), description: '' },
            onChange: function(){ bigbuttons(); }
        });

        /* одноразові дії */
        var once = false;
        Lampa.Settings.listener.follow('open', function(e){
            if (e.name === 'maxsm_themes' && !once){
                once = true;
                fix_lang();
                forall();
                incardtemplate();
    ...
    /* маніфест */
    Lampa.Manifest.plugins = { name: 'maxsm_themes', version: '2.6.1', description: 'maxsm_themes' };
    window.maxsm_themes = {};
})();
