(function () {
    ...
    // Отримання RGB для фільтра з урахуванням color_enabled
    ...
    // Перевірка на власний фільтр в SVG
    ...
    // Вставка прев'ю лоадера в налаштування
    ...
    function remove_activity_loader() {
        ...
        // Видаляємо стиль модального, якщо існує
        var modalStyle = document.getElementById('aniload');
        if (modalStyle) modalStyle.remove();
        $('#pause-icon-custom-color').remove();
    }
    // Створення стилів для модального вікна
    function create_ani_modal() {
        var colorEnabled = Lampa.Storage.get('color_plugin_enabled', 'false') === 'true';
        if (!colorEnabled) return;

        if (document.getElementById('aniload')) return; // Не додаємо дублі
        ...
        style.textContent = 
            ...
            '.ani_loader_square { width: 35px; height: 35px; border-radius: 4px; border: 1px solid #ddd; display: flex; flex-direction: column; justify-content: center; align-items: center; cursor: pointer; color: #ffffff !important; font-size: 10px; text-align: center; }' +
            ...
            '.ani_loader_square.default { width: 35px; height: 35px; border-radius: 4px; border: 1px solid #ddd; }' +
    ...
    // Додавання фокус-лістенера для прев'ю
    ...
    function aniLoad() {
        ...
        try {
            Lampa.SettingsApi.addParam({
                ...
                onChange: function (value) {
                    var colorEnabled = Lampa.Storage.get('color_plugin_enabled', 'false') === 'true';
                    if (!colorEnabled && value === 'true') {
                        Lampa.Noty.show('Для анімації завантаження потрібен увімкнений плагін кольорів');
                        value = 'false';
                    }
                    Lampa.Storage.set('ani_active', value);
                    var selectItem = document.querySelector('.settings-param[data-name="select_ani_mation"]');
                    if (selectItem) {
                        selectItem.style.display = (value === 'true' && colorEnabled) ? 'block' : 'none';
                        if (value === 'true') {
                ...
                onRender: function (item) {
                    var colorEnabled = Lampa.Storage.get('color_plugin_enabled', 'false') === 'true';
                    if (item.style) {
                        item.style.display = colorEnabled ? 'block' : 'none';
                    }
                }
            });
        } catch (e) {}
        try {
            Lampa.SettingsApi.addParam({
                ...
                onRender: function (item) {
                    var colorEnabled = Lampa.Storage.get('color_plugin_enabled', 'false') === 'true';
                    var aniActive = Lampa.Storage.get('ani_active', 'false') === 'true';
                    if (item.style) {
                        item.style.display = (aniActive && colorEnabled) ? 'block' : 'none';
                    }
                    if (aniActive && colorEnabled) {
                        setTimeout(function () {
                ...
                onChange: function () {
                    var colorEnabled = Lampa.Storage.get('color_plugin_enabled', 'false') === 'true';
                    if (!colorEnabled) {
                        Lampa.Noty.show('Для вибору анімації потрібен увімкнений плагін кольорів');
                        return;
                    }
        ...
        // Mutation Observer для динамічних елементів
        setTimeout(function () {
        ...
        // Лістенери для повноекранного режиму
        ...
        // Інтервал для деактивації лоадера
        setInterval(function () {
            var element = document.querySelector('.activity__loader');
    ...
    // Функція оновлення при зміні кольору
