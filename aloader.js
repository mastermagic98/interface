(function () {
    ...
    // Додаємо шаблон для модального вікна (двоколонкова структура)
    ...
    function setCustomLoader(url) {
        ...
        // Отримуємо колір виділення з th3.js
        ...
        var newStyle = '.activity__loader { display: none !important; }' +
                       '.activity__loader.active { ' +
                           ...
                           '-webkit-mask-image: url(\'' + escapedUrl + '\') !important; ' +
    ...
    function aniLoad() {
        ...
        // Ініціалізація видимості параметра select_ani_mation
        updateAniVisibility();
    }
    ...
    // Періодична перевірка для приховування активного завантажувача
    setInterval(function () {
        var element = document.querySelector('.activity__loader');
        if (element && element.classList.contains('active') && Lampa.Storage.get('ani_active', 'false') !== 'true') {
            element.classList.remove('active');
            element.style.display = 'none';
        }
    }, 500);
    ...
    if (window.appready) {
        ...
        if (window.svg_loaders && window.svg_loaders.length > 0) {
            ...
            // Завантажуємо loaders.js, якщо svg_loaders недоступний
    ...
    // Слухаємо зміни акцентного кольору
    Lampa.Storage.listener.follow('change', function (e) {
        if (e.name === 'color_plugin_main_color') {
            byTheme();
        }
