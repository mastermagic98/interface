(function () {
    ...
    function getColorFilter(mainColor) {
        if (!mainColor || mainColor === '#ffffff') return '';
        var rgb = mainColor.toLowerCase() === '#353535' ? { r: 255, g: 255, b: 255 } : hexToRgb(mainColor);
        var r = (rgb.r / 255).toFixed(4);
        var g = (rgb.g / 255).toFixed(4);
        var b = (rgb.b / 255).toFixed(4);
        return 'url("data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cfilter id=%22recolor%22 color-interpolation-filters=%22sRGB%22%3E%3CfeColorMatrix type=%22matrix%22 values=%220 0 0 0 ' + r + ' 0 0 0 0 ' + g + ' 0 0 0 0 ' + b + ' 0 0 0 1 0/%3E%3C/filter%3E%3C/svg%3E#recolor")';
    }
    ...
    function applyDefaultLoaderColor() {
        var defaultSvg = '<?xml version="1.0" encoding="utf-8"?>' +
                         ...
                         '<circle cx="50" cy="50" fill="none" stroke="%23ffffff" stroke-width="5" r="35" stroke-dasharray="164.93361431346415 56.97787143782138">' +
    ...
    function setCustomLoader(url) {
        ...
        var newStyle = 
            ...
            'body .activity__loader.active { background-image: url(\'' + escapedUrl + '\') !important; background-repeat: no-repeat !important; background-position: 50% 50% !important; background-size: contain !important; background-color: transparent !important; filter: ' + filterValue + ' !important; display: block !important; position: fixed !important; left: 50% !important; top: 50% !important; transform: translate(-50%, -50%) scale(1) !important; -webkit-transform: translate(-50%, -50%) scale(1) !important; width: 108px !important; height: 108px !important; z-index: 9999 !important; }' +
            'body .lampac-balanser-loader.custom { background-image: url(\'' + escapedUrl + '\') !important; background-repeat: no-repeat !important; background-position: 50% 50% !important; background-size: contain !important; background-color: transparent !important; filter: ' + filterValue + ' !important; }' +
            ...
            'body .modal-loading.custom { background-image: url(\'' + escapedUrl + '\') !important; background-repeat: no-repeat !important; background-position: 50% 50% !important; background-size: contain !important; background-color: transparent !important; filter: ' + filterValue + ' !important; display: block !important; z-index: 9999 !important; }';
    ...
    function insert_activity_loader_prv(escapedUrl) {
        ...
        var newStyle = '.settings-param[data-name="select_ani_mation"] .activity__loader_prv { display: inline-block; width: 23px; height: 24px; margin-right: 10px; vertical-align: middle; background: url(\'' + escapedUrl.replace(/'/g, "\\'") + '\') no-repeat 50% 50% / contain transparent; color: #ffffff !important; filter: brightness(0) invert(1) !important; }' +
                       'body.glass--style .settings-param.focus .settings-folder__icon .activity__loader_prv { filter: brightness(0) invert(1) !important; }';
        ...
        setInterval(function () {
            if (Lampa.Storage.get('ani_load') && Lampa.Storage.get('ani_active') && Lampa.Storage.get('ani_load') !== './img/loader.svg') {
                ...
                if (playerVideo) {
                    ...
                    if (loader && getComputedStyle(loader).backgroundImage.includes('loader.svg')) {
                        setCustomLoader(Lampa.Storage.get('ani_load'));
                    }
                    var youtubeNeedclick = playerVideo.querySelector('.player-video__youtube-needclick > div');
                    if (youtubeNeedclick && getComputedStyle(youtubeNeedclick).backgroundImage.includes('loader.svg')) {
                        setCustomLoader(Lampa.Storage.get('ani_load'));
                    }
                    var modalLoading = playerVideo.querySelector('.modal-loading');
                    if (modalLoading && getComputedStyle(modalLoading).backgroundImage.includes('loader.svg')) {
                        setCustomLoader(Lampa.Storage.get('ani_load'));
                    }
                }
                var modalLoading = document.querySelector('.modal-loading');
